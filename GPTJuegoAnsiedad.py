...
clicker_pressed = False  
random_fact = ""
show_fact = False  # Controla si se muestra el dato curioso
fact_start_time = 0  # Guarda el tiempo de inicio de la ventana del dato curioso
...
def speak(text):
    ...
    pygame.mixer.music.set_volume(0)  # Mutea la música de fondo
    engine.say(text)
    engine.runAndWait()
    pygame.mixer.music.set_volume(0.5)  # Restaura la música
...
def show_fact_window():
    """Muestra la ventana del dato curioso con un botón de cierre."""
    ...
    for word in words:
        if font.size(current_line + " " + word)[0] < 460:
            current_line += " " + word
        else:
    ...
    y_offset = 60
    for line in lines:
        message_surface = small_font.render(line.strip(), True, BLACK)
        fact_window.blit(message_surface, (20, y_offset))
        y_offset += 30

    draw_button("Cerrar", 200, 200, 100, 40, PASTEL_BLUE, small=True)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            mouse_x, mouse_y = event.pos
            if show_fact and 300 <= mouse_x <= 400 and 325 <= mouse_y <= 365:
                show_fact = False  
            elif 600 <= mouse_x <= 750 and 500 <= mouse_y <= 550 and random.randint(1, 3) == 1:
                clicker_pressed = True  
                counter += 1  
